...
savepath = "/home/shiqinghongya/original_ce/visualization/new/"
...
class ft_net(nn.Module):
    ...
    def forward(self, x):
        if True:  # draw features or not
            ...
            # draw_features(1, 1, x.cpu().numpy(), "{}/f1_conv1.png".format(savepath))
            draw_features(8, 8, x.cpu().numpy(), "{}/f1_conv1.png".format(savepath))

            x = self.model.bn1(x)
            # draw_features(1, 1, x.cpu().numpy(), "{}/f2_bn1.png".format(savepath))
            draw_features(8, 8, x.cpu().numpy(), "{}/f2_bn1.png".format(savepath))

            x = self.model.relu(x)
            # draw_features(1, 1, x.cpu().numpy(), "{}/f3_relu.png".format(savepath))
            draw_features(8, 8, x.cpu().numpy(), "{}/f3_relu.png".format(savepath))

            x = self.model.maxpool(x)
            # draw_features(1, 1, x.cpu().numpy(), "{}/f4_maxpool.png".format(savepath))
            draw_features(8, 8, x.cpu().numpy(), "{}/f4_maxpool.png".format(savepath))

            x = self.model.layer1(x)
            # draw_features(1, 1, x.cpu().numpy(), "{}/f5_layer1.png".format(savepath))
            draw_features(16, 16, x.cpu().numpy(), "{}/f5_layer1.png".format(savepath))

            x = self.model.layer2(x)
            # draw_features(1, 1, x.cpu().numpy(), "{}/f6_layer2.png".format(savepath))
            draw_features(16, 32, x.cpu().numpy(), "{}/f6_layer2.png".format(savepath))

            x = self.model.layer3(x)
            # draw_features(1, 1, x.cpu().numpy(), "{}/f7_layer3.png".format(savepath))
            draw_features(32, 32, x.cpu().numpy(), "{}/f7_layer3.png".format(savepath))

            x = self.model.layer4(x)
            # draw_features(1, 1, x.cpu().numpy(), "{}/f8_layer4_2.png".format(savepath))
...
img = cv2.imread('/home/shiqinghongya/original_ce/visualization/0dc1f4d86da3f8461bb5c5258b88ce64.jpg')
