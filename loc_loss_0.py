...
MASK_ROOT = "/home/shiqinghongya/intention/annotations"
...
class Localizationloss(nn.Module):
    ...
    def _get_CAM(self, model, X, cls_idx):
        ...
        if isinstance(X, tuple):  # image + hs
            ...
            layer = model.module.model._modules.get('7')
        ...
        with GradCam(model, [layer]) as gcam:
            ...
            out_b = gcam(X)
            # out_b[:, cls_idx].mean().backward()
            out_b = [x[cls_idx] for x in out_b]
...
class GradCam:
    ...
    def __init__(self, model, layers):
        ...
        def forward_hook(module, input, output):
            ...
            self.fmap_pool[module] = output.data.cpu().numpy()

        def backward_hook(module, grad_in, grad_out):
            # self.grad_pool[module] = grad_out[0].detach().cpu()
            self.grad_pool[module] = grad_out[0].data.cpu().numpy()

        for layer in layers:
            self.hooks.append(layer.register_forward_hook(forward_hook))
            self.hooks.append(layer.register_backward_hook(backward_hook))
            # self.hooks.extend(layer.register_forward_hook(forward_hook))
            # self.hooks.extend(layers.register_backward_hook(backward_hook))
        # self.close()
