...
class GradCam:
    ...
    def get(self, layer):
        ...
        fmap_b = []
        grad_b = []
        for i in self.fmap_pool.keys():
            fmap_b.extend(self.fmap_pool[i])
        for j in self.grad_pool.keys():
            grad_b.extend(self.grad_pool[j])
        fmap_b = torch.stack(fmap_b, 0)
        grad_b = torch.stack(grad_b, 0)
